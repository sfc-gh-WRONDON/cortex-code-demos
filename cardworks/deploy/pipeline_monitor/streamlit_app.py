...
# ─── Connection (SiS uses embedded identity) ─────────────────────────────────
...
def generate_ai_summary(failures_df: pd.DataFrame, anomalies_df: pd.DataFrame) -> str:
    ...
    return result.iloc[0]["SUMMARY"] if not result.empty else "Unable to generate summary."
    ...
    for _, row in summary.iterrows():
        ...
        with st.expander(label, expanded=expanded):
            ...
            with cb:
                ...
                if vol == 0 or vol > 150 or (0 < vol < 70):
                    vol_badge = "🔴"
...
with fc1:
    status_filter = st.multiselect("Status", ["SUCCESS", "WARNING", "FAILED"], default=["FAILED", "WARNING"])
with fc2:
    pipeline_filter = st.multiselect("Pipeline", options=sorted(recent_runs["PIPELINE_NAME"].unique().tolist()), default=[])
with fc3:
...
st.caption(f"Showing {len(filtered):,} runs")
